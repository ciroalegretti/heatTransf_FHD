...
def volumesArrangement(volNumb,volNodes,volArr):       
    ...
    Conectivity matriz:
        ...
        domain boundaries) and a ghost volume will be placed later
...
def exportData(CVdata,wallFunc,volNumb,volNodes,volArr,nodesCoord,nodeNumb,nx1,ny1,x1,y1,Le,h):
    ...
    """    
    np.savez('./PP/gridData_L={:.1f}_h={}_nx={}_ny={}_{}volumes.npz'.format(Le,h,nx1,ny1,volNumb), nx1=nx1, ny1=ny1, volArr=volArr, wallFunc = wallFunc, volNodes=volNodes, nodesCoord=nodesCoord, \
                             x1=x1, y1=y1, CVdata=CVdata, volNumb=volNumb, nodeNumb=nodeNumb,Le=Le,h=h)
    
    myFile = open('./PP/FVgrid_L={:.1f}_h={}_nx={}_ny={}_{}volumes.dat'.format(Le,h,nx1,ny1,volNumb), 'w')
