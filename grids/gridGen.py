...
Ls = 30*h  # Lenght of expansion channel

for n in np.linspace(3,4,5):#,6,7,8,9,10]:
    #    for beta in betaArr:
    """Numerical parameters"""
    nx1 = int(50*n)#int(45*1.1**(2*n))
    ny1 = int(20*n)#int(30*1.1**(2*n))
    ##################################################
    nx2 = int(120*n)#3*nx1
    ny2 = ny1
    nx3 = nx2
    ny3 = int(1.25*ny1)       # doimain 3 with 50% more nodes in y-direction than domain 2 for ER=2 and proportional when ER=1.5 and 3.0
    ...
    x1 = gridGenFunc.strechX(beta,x1,Le,Le,-1,d=0.01*Le)
