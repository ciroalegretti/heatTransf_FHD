...
# @jit(nopython=True)
def macCormack_psi(data,flux,volArr,cv,dt):
    """ Solves pseudo transient poisson equation using explicit MacCormack method"""
    ...
    fluxdiffP = fluxPsi.fluxdiffPsi(flux,data,cv,volArr)
    
    for i in range(n):
        dxV = cv[i,4]#nodesCoord[trn,1] - nodesCoord[tln,1]
        dyV = cv[i,5]#nodesCoord[trn,2] - nodesCoord[brn,2]
        
        derP[i,0] = - (fluxdiffP[i,0] - fluxdiffP[i,2])/dxV \
                    - (fluxdiffP[i,1] - fluxdiffP[i,3])/dyV    

        dataP[i,3] = data[i,3] + derP[i,0]*dt

    # Corrector step (backward approximation for advection fluxes)

    fluxdiffC = fluxPsi.fluxdiffPsi(fluxdiffP,dataP,cv,volArr)
    
    for i in range(n):
        dxV = cv[i,4]#nodesCoord[trn,1] - nodesCoord[tln,1]
        dyV = cv[i,5]#nodesCoord[trn,2] - nodesCoord[brn,2]
                                    
        derC[i,0] = - (fluxdiffC[i,0] - fluxdiffC[i,2])/dxV \
                    - (fluxdiffC[i,1] - fluxdiffC[i,3])/dyV  

        derAVG[i,0] = (derP[i,0] + derC[i,0])/2
        
    # for i in range(n):        
        dataC[i,3] = data[i,3] + derAVG[i,0]*dt - data[i,4]
        
        # print(i,dataC[i,3])
        
    return dataC, fluxdiffC

@jit(nopython=True)
def macCormack_XI(data,c,flux,s,wf,volArr,cv,dt):
