...
def vort_cg(volnumb,volarr,cv,h_data,dt,re,tol=1e-6):
    ...
    for i in prange(volnumb):
        ...
        r[i] = b[i] - (1./dt + (1./re)*(1./((cv[E,2] - cv[W,2])*(cv[E,2] - cv[i,2])) + 1./((cv[E,2] - cv[W,2])*(cv[i,2] - cv[W,2])) + 1./((cv[N,3] - cv[S,3])*(cv[N,3] - cv[i,3])) + 1./((cv[N,3] - cv[S,3])*(cv[i,3] - cv[S,3]))))*Wo[i] \
                    - (- 1./(re*(cv[E,2] - cv[W,2])*(cv[E,2] - cv[i,2])))*Wo[E]  \
                    - (- 1./(re*(cv[E,2] - cv[W,2])*(cv[i,2] - cv[W,2])))*Wo[W]  \
                    - (- 1./(re*(cv[N,3] - cv[S,3])*(cv[N,3] - cv[i,3])))*Wo[N]  \
                    - (- 1./(re*(cv[N,3] - cv[S,3])*(cv[i,3] - cv[S,3])))*Wo[S]  \
                    - (Upos*wBaX + Uneg*wFoX)                                 \
                    - (Vpos*wBaY + Vneg*wFoY)                                  
    ...
    while it_number < it_max and l2_norm > tol:
        ...
        for i in prange(volnumb):
            ...
            q[i] =  + (1./dt + (1./re)*(1./((cv[E,2] - cv[W,2])*(cv[E,2] - cv[i,2])) + 1./((cv[E,2] - cv[W,2])*(cv[i,2] - cv[W,2])) + 1./((cv[N,3] - cv[S,3])*(cv[N,3] - cv[i,3])) + 1./((cv[N,3] - cv[S,3])*(cv[i,3] - cv[S,3]))))*d[i] \
                    + (- 1./(re*(cv[E,2] - cv[W,2])*(cv[E,2] - cv[i,2])))*d[E]  \
                    + (- 1./(re*(cv[E,2] - cv[W,2])*(cv[i,2] - cv[W,2])))*d[W]  \
                    + (- 1./(re*(cv[N,3] - cv[S,3])*(cv[N,3] - cv[i,3])))*d[N]  \
                    + (- 1./(re*(cv[N,3] - cv[S,3])*(cv[i,3] - cv[S,3])))*d[S]  \
                    + (Upos*dBaX + Uneg*dFoX)                                 \
                    + (Vpos*dBaY + Vneg*dFoY)                          
